...
namespace ASD
{
    ...
    public class FindShortestPathTestCase : TestCase
    {
        ...
        int expectedResult;
        int t;
        bool withDynamites = true;
        string path;

        int result;
        bool checkPath;

        public FindShortestPathTestCase(double timeLimit, Exception expectedException, string description, char[,] M, bool withDynamite, int t, int expectedResult, bool checkPath)
            : base(timeLimit, expectedException, description)
        {
            this.M = (char[,])M.Clone();
            this.withDynamites = withDynamite;
            this.t = t;
        ...
        protected override (Result resultCode, string message) VerifyTestCase(object settings)
        {
            ...
            if (checkPath)
            {
                ...
                for (int i = 0; i < path.Length; i++)
                {
                    switch (path[i])
                    {
                        case 'N': curr.Item1--; break;
                        case 'S': curr.Item1++; break;
                        case 'E': curr.Item2++; break;
                        case 'W': curr.Item2--; break;
                        default:
    ...
    public class FindShortestPathWithKDynamites : TestCase
    {
        ...
        public FindShortestPathWithKDynamites(double timeLimit, Exception expectedException, string description, char[,] M, int k, int t, int expectedResult, bool checkPath)
            : base(timeLimit, expectedException, description)
        {
            this.M = (char[,])M.Clone();
            this.k = k;
